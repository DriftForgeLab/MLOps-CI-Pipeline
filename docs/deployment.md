...
- At least one model promoted to **Production** via the pipeline
  (`run-pipeline --config src/config/pipeline.yaml` and approve when prompted).
...
| `PIPELINE_CONFIG_PATH`| `/app/src/config/pipeline.yaml` | Pipeline config path inside the container.       |
