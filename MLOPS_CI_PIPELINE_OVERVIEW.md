...
2. **Tabular regression** (`task_type: regression`) on CSV datasets, using scikit-learn estimators (Histogram Gradient Boosting Regressor as the default, with Linear Regression as an alternative).
...
- `src/monitoring/` exposes the operational CLIs `monitor-drift`, `monitor-drift-image`, `monitor-summary`, `prepare-image-batch`, `prepare-drift-training`, and `prepare-drift-training-tabular`. It also contains the drift history index ([src/monitoring/history.py](src/monitoring/history.py)), a standalone MLflow sink for runtime drift ([src/monitoring/mlflow_sink.py](src/monitoring/mlflow_sink.py)), the decision gate ([src/monitoring/drift_decision.py](src/monitoring/drift_decision.py)), and reporting helpers ([src/monitoring/reports.py](src/monitoring/reports.py)).
...
### 3.8 Drift-Adaptive Evaluation Pre-Promotion Hook (image fine-tune and tabular retrain runs)

This hook is **not** a stage in `_STAGE_REGISTRY`; it is wired directly into the orchestration loop in [src/pipeline/run_pipeline.py:131-162](src/pipeline/run_pipeline.py#L131-L162). It fires immediately before the `promotion` stage under two conditions: (a) `--fine-tune` is set (image CNN path), or (b) the `.drift_prepared` marker file exists at `data/evaluation/drifted_holdout/<dataset>/.drift_prepared` (tabular path). In both cases `run_drift_adaptation_eval` ([src/pipeline/steps.py:484-590](src/pipeline/steps.py#L484-L590)) runs the same underlying logic:

1. Looks for a holdout directory at `data/evaluation/drifted_holdout/<dataset>/` and a sibling `baseline_metrics.json` written by `prepare-drift-training` or `prepare-drift-training-tabular`. Returns `None` (silent skip) if either is missing.
2. Loads the newly trained/fine-tuned model from `artifacts/runs/<version_id>/model/`.
3. Loads preprocessing config and normalisation stats (either saved alongside the holdout or from the current version).
4. Runs the holdout evaluation, computes a per-metric delta against the baseline, and returns `{baseline, after_finetuning, delta, improved}`.
5. Writes `outputs/drift_adaptation_eval.json` and logs `drift_adapt.delta_*` metrics to the active MLflow run.

**Stale-result protection.** At the very start of every `run-pipeline` invocation, any existing `outputs/drift_adaptation_eval.json` is unconditionally deleted. For the tabular path, the `.drift_prepared` marker is also deleted after the hook fires, so the drift-adaptation block only appears at the promotion gate immediately after a `prepare-drift-training-tabular` run — not on subsequent normal pipeline runs.

The subsequent `_promotion_stage` reads `drift_adaptation_eval.json` (if present) and passes it to the approval gate so the approver sees a concrete before/after comparison on the held-out drifted data.
...
Online drift detection is implemented in [src/monitoring/drift.py](src/monitoring/drift.py) (tabular) and [src/monitoring/image_drift_monitor.py](src/monitoring/image_drift_monitor.py) (image). These are invoked manually after real production data is collected via the CLIs `monitor-drift`, `monitor-drift-image`, and `monitor-summary`. Tabular drift uses Evidently's `DataDriftPreset` plus per-feature extraction in [src/drift/compute.py](src/drift/compute.py); image drift uses per-channel Wasserstein distance computed at multiple spatial resolutions via a **Gaussian pyramid** (`compute_multiscale_drift` in [src/drift/image_compute.py](src/drift/image_compute.py)). The pyramid builds three resolution levels (1/1, 1/2, 1/4 of the original) from both the batch and reference images and computes Wasserstein distance at each level independently. A `scale_decay_ratio` close to 1.0 indicates drift is global and coarse (e.g. brightness/colour shifts that persist at all scales), while a decaying ratio indicates localised or high-frequency drift. For raw-ISP pipelines the drift result is additionally matched against pre-computed ISP scenario signatures to identify the most likely physical cause of the shift.
...
If severity exceeds the configured `alert_severity`, the user is prompted through [src/monitoring/drift_decision.py](src/monitoring/drift_decision.py) to choose a response. The available options depend on pipeline type: **image CNN pipelines** offer `fine_tune` (update weights from Production), `retrain`, `collect_data`, `accept`; **tabular pipelines** offer `retrain_merged` (retrain from scratch on original + drifted data), `retrain` (original data only), `collect_data`, `accept`; **raw-ISP pipelines** additionally offer `adjust_isp` (re-run preprocessing with corrected ISP parameters). Retraining itself is not automated — it requires re-invoking `run-pipeline`.

The recommended response to detected **image** drift is: organise the drifted batch into an ImageFolder layout, run `prepare-drift-training` to split it into a training portion (added to the raw dataset) and a held-out evaluation set with a Production-model baseline, then run `run-pipeline --fine-tune` so the pre-promotion hook (§3.8) can produce an automatic before/after comparison for the approver.

The recommended response to detected **tabular** drift is: run `prepare-drift-training-tabular --drifted-csv <path> --config <config>` to split the labeled drifted CSV into a training portion (appended to `data/raw/<dataset>/data.csv`) and a held-out evaluation set with a baseline, then run `run-pipeline --config <config>` (without `--fine-tune`) to retrain from scratch on the merged data. The orchestrator detects the `.drift_prepared` marker created by `prepare-drift-training-tabular` and runs the holdout evaluation automatically before the promotion gate (see §3.8).
...
| `prepare-drift-training-tabular` | `src.monitoring.prepare_drift_tabular_cli:main` | Prepare labeled drifted CSV for tabular drift-adaptive retraining (split, baseline eval, append to raw dataset) |
...
- **Manual / local**: `run-pipeline --config src/config/pipeline_tabular_classification.yaml` from the command line. The approval gate in `_promotion_stage` requires `stdin.isatty()` for interactive input; in headless environments `input()` receives an `EOFError` and is treated as a cancellation, so the pipeline exits before registering a model.
...
5. **Run pipeline**: `run-pipeline --config src/config/pipeline_tabular.yaml`. Note: this config path refers to the old filename — the current correct name is `pipeline_tabular_classification.yaml`. The CI workflow has not been updated to use the renamed file (**Gap — the CI workflow references a filename that no longer exists after the rename**). This executes preprocessing → training → evaluation → promotion on the tabular reference dataset. Because stdin is not a TTY in CI, the approval gate cancels and the promotion stage will not actually write a Production model to the registry (**Strong inference**).
...
| `scikit-learn` | `1.4.2` | Random Forest / Logistic Regression / `HistGradientBoostingRegressor` (default regression) / Linear Regression, `train_test_split`, `ColumnTransformer`, `Pipeline`, scalers, encoders, metrics |
...
- **Drift-adaptive retraining (image)** — `prepare-drift-training` splits a labeled drifted image batch into training and holdout sets, evaluates the Production model on the holdout as a baseline, then copies training images into the raw dataset; `run-pipeline --fine-tune` continues training from Production weights and automatically re-evaluates on the same holdout before the approval gate.
- **Drift-adaptive retraining (tabular)** — `prepare-drift-training-tabular` splits a labeled drifted CSV into a training portion (appended to `data/raw/<dataset>/data.csv`) and a held-out evaluation set with a Production-model baseline; `run-pipeline` (without `--fine-tune`) retrains from scratch on the merged data and automatically re-evaluates on the holdout before the approval gate, gated by a `.drift_prepared` marker file.
- **Lineage tagging** at the model-version level with hard-fail validation.
- **Drift detection**: Evidently-based KS / chi-square for tabular; per-channel Wasserstein distance computed at multiple resolutions via a **Gaussian pyramid** (3 levels: 1/1, 1/2, 1/4) for images ([src/drift/image_compute.py](src/drift/image_compute.py)). The `scale_decay_ratio` output distinguishes global/coarse drift (brightness, colour cast — ratio ≈ 1.0) from fine-grained / local drift (ratio decays across levels).
...
| **Drift-adaptive retraining** | Present | Image: `prepare-drift-training` + `run-pipeline --fine-tune`. Tabular: `prepare-drift-training-tabular` + `run-pipeline`. Both paths produce an automatic holdout before/after comparison at the promotion gate; human-initiated, not automated. |
...
A distinctive research dimension is the treatment of *image drift*. The project integrates a deterministic seven-stage Image Signal Processing pipeline (black-level correction → demosaicing → white balance → colour correction → denoising → sharpening → gamma), optionally GPU-accelerated, which is used both for pre-deployment sensitivity analysis and for physically motivated drift scenarios (warm/cool white balance, gamma shifts, noise changes, heavy sharpening). Tabular drift is handled through Evidently's `DataDriftPreset` with statistical tests and configurable severity levels; image drift is measured by per-channel Wasserstein distance computed at multiple spatial resolutions via a Gaussian pyramid (three levels), with a `scale_decay_ratio` that distinguishes global colour/brightness shift from fine-grained local drift. For raw-ISP pipelines, drift is additionally matched against pre-computed scenario signatures to attribute the shift to a physical ISP cause. Runtime drift results are persisted to a JSONL history index and logged to **separate** MLflow runs so training provenance remains immutable. Two *drift-adaptive retraining* workflows close the loop from drift detection back to model improvement: for image CNN pipelines, `prepare-drift-training` followed by `run-pipeline --fine-tune` continues training from Production weights; for tabular pipelines, `prepare-drift-training-tabular` followed by `run-pipeline` retrains from scratch on the merged original and drifted data. Both workflows produce an automatic held-out before/after evaluation surfaced to the human approver at the promotion gate.
